...
export class GraphService {
  ...
  async deleteSubscription(subscriptionId: string): Promise<void> {
    try {
      ...
      // If 404, subscription doesn't exist in Graph (might already be deleted)
      ...
      this.logger.error(
        `Failed to delete subscription ${subscriptionId} from Microsoft Graph: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async verifyMailboxExists(email: string): Promise<void> {
    try {
      ...
      // For other errors, log and rethrow
      this.logger.error(`Error verifying mailbox ${email}: ${error.message}`);
      throw error;
    }
  }
  ...
  async verifyDomainExists(domain: string): Promise<void> {
    try {
      ...
      // If 404, domain doesn't exist in the tenant
      if (error.response?.status === 404) {
        throw new BadRequestException(
      ...
      // For other errors, log and rethrow
      this.logger.error(`Error verifying domain ${domain}: ${error.message}`);
      throw error;
    }
  }
  ...
  async createSubscriptionsForMailbox(mailboxEmail: string): Promise<{
    ...
    if (!existingInboxSub) {
      try {
        ...
        this.logger.error(
          `Failed to create inbox subscription for ${mailboxEmail}: ${error.message}`,
        );
    ...
    if (!existingSpamSub) {
      try {
        ...
        this.logger.error(
          `Failed to create spam subscription for ${mailboxEmail}: ${error.message}`,
        );
        // Don't throw - allow mailbox creation to succeed even if subscription fails
      }
    } else {
  ...
  async sendEmail(
    ...
    try {
      if (inReplyTo) {
        ...
        if (cc && cc.length > 0) {
          message.message.ccRecipients = cc.map((r) => ({
            emailAddress: {
              address: r.emailAddress,
              name: r.displayName || null,
            },
        ...
        if (bcc && bcc.length > 0) {
          message.message.bccRecipients = bcc.map((r) => ({
            emailAddress: {
              address: r.emailAddress,
              name: r.displayName || null,
            },
      ...
      this.logger.error(
        `Failed to send email from ${mailboxEmail}. Status: ${statusCode}, Error: ${JSON.stringify(errorDetails)}`,
      );

      // Create a more descriptive error
      const descriptiveError = new Error(
        `Failed to send email: ${errorMessage}${statusCode ? ` (HTTP ${statusCode})` : ''}`,
      );
      (descriptiveError as any).statusCode = statusCode;
      (descriptiveError as any).originalError = errorDetails;
