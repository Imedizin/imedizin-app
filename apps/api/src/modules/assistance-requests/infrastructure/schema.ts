...
export const assistanceRequests = pgTable(
  ...
  (t) => ({
    ...
    providerRefIdx: index(
      "idx_assistance_requests_provider_reference_number",
    ).on(t.providerReferenceNumber),
    receivedAtIdx: index("idx_assistance_requests_received_at").on(
      t.receivedAt,
    ),
...
export const transportRequests = pgTable("transport_requests", {
  ...
  medicalCrewRequired: boolean("medical_crew_required")
    .notNull()
    .default(false),
