...
import type { ThreadSummary, EmailDetail } from "@/types/email";
...
const MailsThreaded: React.FC = () => {
  ...
  const {
    ...
    dataUpdatedAt,
  } = useGetThreadsQuery(selectedMailboxId || undefined, currentPage, pageSize);
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {isLoadingThreads ? (
              ...
              threads.map((thread: ThreadSummary) => (
                <div
                  key={thread.threadId}
                  onClick={() => navigate(`/mails-threaded/${thread.threadId}`)}
                  style={{
                    display: "flex",
