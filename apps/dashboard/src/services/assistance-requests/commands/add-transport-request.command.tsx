...
function buildTransportPayload(
  ...
  return {
    requestNumber: values.requestNumber ?? `TR-${Date.now()}`,
...
export const useAddTransportRequestCommand = () => {
  ...
  const addMutation = useMutation({
    mutationFn: async (
      ...
      const result = await apiClient
        .post("assistance-requests/transport", { json: payload })
    ...
    onSuccess: () => {
      ...
      message.success("Transportation request created");
    },
    onError: (error: Error) => {
